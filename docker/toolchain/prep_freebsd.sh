...
mkdir /tmp/freebsdbuild && cd /tmp/freebsdbuild && \
  wget --no-verbose --tries=3 https://ftp.gnu.org/gnu/binutils/binutils-${binutils_ver}.tar.xz && \
...
cd /tmp/freebsdbuild && \
  wget --no-verbose --tries=3 https://download.freebsd.org/ftp/releases/amd64/${freebsd_full_ver}-RELEASE/base.txz && \
  ...
  find . -xtype l|xargs ls -l|grep ' /lib/' \
    | awk '{print "ln -sf /freebsdcross/x86_64-pc-freebsd'${freebsd_ver}'"$11 " " $9}' \
    | /bin/sh && \
  rm -rf /tmp/freebsdbuild && mkdir /tmp/freebsdbuild

# Compile GMP
cd /tmp/freebsdbuild && \
  wget --no-verbose --tries=3 https://ftp.gnu.org/gnu/gmp/gmp-${gmp_ver}.tar.xz && \
...
# Compile MPFR
cd /tmp/freebsdbuild && \
  wget --no-verbose --tries=3 https://ftp.gnu.org/gnu/mpfr/mpfr-${mpfr_ver}.tar.xz && tar -xf mpfr-${mpfr_ver}.tar.xz && \
  cd mpfr-${mpfr_ver} && \
...
# Compile MPC
cd /tmp/freebsdbuild && \
  wget --no-verbose --tries=3 https://ftp.gnu.org/gnu/mpc/mpc-${mpc_ver}.tar.gz && tar -xf mpc-${mpc_ver}.tar.gz && \
  cd mpc-${mpc_ver} && \
  ./configure --prefix=/freebsdcross/x86_64-pc-freebsd${freebsd_ver} --with-gnu-ld --enable-static \
...
cd /tmp/freebsdbuild && \
  wget --no-verbose --tries=3 https://ftp.gnu.org/gnu/gcc/gcc-${gcc_ver}/gcc-${gcc_ver}.tar.xz && \
